...
const LoginEnrollment = ({ setIsAuthenticated }) => {
  ...
  const verifyOtp = async () => {
    try {
      setLoading3(true);
      await axios.post(`${API_BASE_URL}/auth/verify-otp`, { email: tempLoginData.email, otp: otp.join("") });
  ...
  const resendOtp = async () => {
    try {
      setLoading2(true);
      await axios.post(`${API_BASE_URL}/auth/request-otp`, { email: tempLoginData.email });
